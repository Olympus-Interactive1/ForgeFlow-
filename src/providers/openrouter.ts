...
export const openRouterProvider: Provider = {
  ...
  async discoverModels(context?: ProviderContext): Promise<DiscoveredModel[]> {
    ...
    for (const model of catalog.data ?? []) {
      ...
      if (input.includes('image')) capabilities.push('image');
      if (output.includes('image')) capabilities.push('image');
  ...
  async execute(request: ModelRequest, context: ProviderContext): Promise<ModelResponse> {
    ...
    if (operation === 'image_generate' || operation === 'image_edit') {
      ...
      return { output: item.b64_json ? { data: item.b64_json, mimeType: item.media_type ?? 'image/png' } : { url: item.url }, provider: 'openrouter', model, metadata: { free: false, endpoint: 'images' } };
    }

    if (operation === 'video_generate' || operation === 'video_image_to_video') {
      const options = (request.metadata?.videoOptions ?? {}) as Record<string, unknown>;
      const body: Record<string, unknown> = { model, prompt: request.prompt ?? String(request.input ?? ''), ...options };
      if (request.input && typeof request.input === 'string' && request.input.startsWith('data:image/')) {
        body.frame_images = [{ frame_type: 'first_frame', image_url: request.input }];
      }
      ...
      for (let attempt = 0; attempt < 90; attempt++) {
        ...
        if (status.status === 'failed' || status.status === 'cancelled') throw new Error(status.error ?? `OpenRouter video generation ${status.status}`);
        await new Promise(resolve => setTimeout(resolve, 2000));
        status = await requestJson<VideoJob>(pollUrl, { method: 'GET', timeoutMs: context.timeoutMs ?? timeout(), headers });
      }
      ...
      return { output: { data: Buffer.from(content.bytes).toString('base64'), mimeType: content.contentType ?? 'video/mp4' }, provider: 'openrouter', model, usage: status.usage, metadata: { free: false, endpoint: 'videos' } };
    }
