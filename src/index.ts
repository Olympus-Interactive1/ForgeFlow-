...
export function createForgeFlowServer() {
  ...
  server.registerTool('forgeflow_route', { description: 'Route a supported request through ForgeFlow. v0.5.0 uses free providers/models only by default.', inputSchema: { capability: z.enum(['text', 'image', 'video', 'audio', 'stt', 'tts', 'embedding']), ...common } }, async args => routeTool(args.capability, args));
  ...
  server.registerTool('forgeflow_image_analyze', { description: 'Analyze an image using a free vision-capable provider.', inputSchema: common }, args => routeTool('image', { ...args, prompt: args.prompt ?? 'Analyze the supplied image.' }));
  ...
  server.registerTool('forgeflow_video_generate', { description: 'Generate a video with a free-capable provider.', inputSchema: common }, args => routeTool('video', { ...args, metadata: { ...args.metadata, operation: 'video_generate' } }));
  server.registerTool('forgeflow_video_image_to_video', { description: 'Generate video from an image with a free-capable provider.', inputSchema: common }, args => routeTool('video', { ...args, metadata: { ...args.metadata, operation: 'video_image_to_video' } }));
  server.registerTool('forgeflow_video_extend', { description: 'Extend a video with a free-capable provider.', inputSchema: common }, args => routeTool('video', { ...args, metadata: { ...args.metadata, operation: 'video_extend' } }));
  server.registerTool('forgeflow_video_analyze', { description: 'Analyze a video using a free vision-capable provider.', inputSchema: common }, args => routeTool('video', args));
  server.registerTool('forgeflow_audio_tts', { description: 'Convert text to speech using a free-capable provider.', inputSchema: common }, args => routeTool('tts', { ...args, metadata: { ...args.metadata, operation: 'tts' } }));
  server.registerTool('forgeflow_audio_stt', { description: 'Transcribe speech using a free-capable provider.', inputSchema: common }, args => routeTool('stt', { ...args, metadata: { ...args.metadata, operation: 'stt' } }));
  server.registerTool('forgeflow_audio_generate', { description: 'Generate music or sound effects with a free-capable provider.', inputSchema: common }, args => routeTool('audio', { ...args, metadata: { ...args.metadata, operation: 'audio_generate' } }));
